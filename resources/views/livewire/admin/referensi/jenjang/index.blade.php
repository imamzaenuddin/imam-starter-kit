...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public ?string $dikti_id = null;
    public ?string $jenjang_dikti_id = null;
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenjang', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'dikti_id', 'jenjang_dikti_id', 'nama', 'keterangan', 'def', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenjang', 'dapat_ubah')) abort(403);

        $item = Jenjang::findOrFail($id);
        $this->editId = $item->jenjang_id;
        $this->dikti_id = $item->dikti_id ?? '';
        $this->jenjang_dikti_id = $item->jenjang_dikti_id ?? '';
        $this->nama = $item->nama ?? '';
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            'dikti_id' => ($data['dikti_id'] === '' ? null : $data['dikti_id']),
            'jenjang_dikti_id' => ($data['jenjang_dikti_id'] === '' ? null : $data['jenjang_dikti_id']),
        ...
        if ($this->editId) {
            $item = Jenjang::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jenjang::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenjang', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Jenjang::query()
            ->when($this->search, function ($query) {
                // $query->where('jenjang_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenjang_id', 'desc')
            ->paginate(10);
...
<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenjang</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenjang</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenjang_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenjang_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenjang</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Dikti Id</label>
                  <input wire:model="dikti_id" type="text" class="form-control @error('dikti_id') is-invalid @enderror">
                  @error('dikti_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Jenjang Dikti Id</label>
                  <input wire:model="jenjang_dikti_id" type="text" class="form-control @error('jenjang_dikti_id') is-invalid @enderror">
                  @error('jenjang_dikti_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
