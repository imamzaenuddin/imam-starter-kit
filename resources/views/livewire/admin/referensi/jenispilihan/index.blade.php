...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenispilihan', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'urutan', 'singkatan', 'prodi_id', 'ta', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenispilihan', 'dapat_ubah')) abort(403);

        $item = Jenispilihan::findOrFail($id);
        $this->editId = $item->jenis_pilihan_id;
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'ta' => ($data['ta'] === '' ? null : $data['ta'])
        ];

        if ($this->editId) {
            $item = Jenispilihan::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jenispilihan::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/referensi/jenispilihan', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Jenispilihan::query()
            ->when($this->search, function ($query) {
                // $query->where('jenis_pilihan_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenis_pilihan_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Jenispilihan')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenispilihan</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenispilihan</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenis_pilihan_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenis_pilihan_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenispilihan</h5>
          ...
          <form wire:submit="simpan">
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Ta</label>
                  <input wire:model="ta" type="text" class="form-control @error('ta') is-invalid @enderror">
