...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenissekolah', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'satu_group', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenissekolah', 'dapat_ubah')) abort(403);

        $item = Jenissekolah::findOrFail($id);
        $this->editId = $item->jenis_sekolah_id;
        $this->nama = $item->nama;
        $this->satu_group = $item->satu_group;
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'satu_group' => ($data['satu_group'] === '' ? null : $data['satu_group']),
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
        ];

        if ($this->editId) {
            $item = Jenissekolah::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jenissekolah::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenissekolah', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Jenissekolah::query()
            ->when($this->search, function ($query) {
                // $query->where('jenis_sekolah_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenis_sekolah_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Jenissekolah')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenissekolah</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenissekolah</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenis_sekolah_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenis_sekolah_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenissekolah</h5>
          ...
          <form wire:submit="simpan">
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Satu Group</label>
                  <input wire:model="satu_group" type="text" class="form-control @error('satu_group') is-invalid @enderror">
                  @error('satu_group') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
