...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pmbformsyarat', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'urutan', 'nama', 'ada_script', 'script', 'keterangan', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pmbformsyarat', 'dapat_ubah')) abort(403);

        $item = Pmbformsyarat::findOrFail($id);
        $this->editId = $item->pmb_form_syarat_id;
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'ada_script' => ($data['ada_script'] === '' ? null : $data['ada_script']),
        ...
        if ($this->editId) {
            $item = Pmbformsyarat::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Pmbformsyarat::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pmbformsyarat', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Pmbformsyarat::query()
            ->when($this->search, function ($query) {
                // $query->where('pmb_form_syarat_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('pmb_form_syarat_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Pmbformsyarat')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Pmbformsyarat</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Pmbformsyarat</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->pmb_form_syarat_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->pmb_form_syarat_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Pmbformsyarat</h5>
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="script" type="text" class="form-control @error('script') is-invalid @enderror">
                  @error('script') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
