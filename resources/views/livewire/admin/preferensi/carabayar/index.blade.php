...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public ?string $cara_bayar_id = null;
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/carabayar', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'cara_bayar_id', 'kode_id', 'nama', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/carabayar', 'dapat_ubah')) abort(403);

        $item = Carabayar::findOrFail($id);
        $this->editId = $item->cara_bayar_id;
        $this->cara_bayar_id = $item->cara_bayar_id;
    ...
    public function simpan(): void
    {
        ...
        $rules = [
            'cara_bayar_id' => 'required|string|max:255',
            'kode_id' => 'nullable',
            'nama' => 'nullable',
            'is_aktif' => 'boolean'
        ];
        ...
        $payload = [
            'cara_bayar_id' => ($data['cara_bayar_id'] === '' ? null : $data['cara_bayar_id']),
        ...
        if ($this->editId) {
            $item = Carabayar::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Carabayar::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/carabayar', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Carabayar::query()
            ->when($this->search, function ($query) {
                // $query->where('cara_bayar_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('cara_bayar_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Carabayar')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Carabayar</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Carabayar</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->cara_bayar_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->cara_bayar_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Carabayar</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Cara Bayar Id</label>
                  <input wire:model="cara_bayar_id" type="text" class="form-control @error('cara_bayar_id') is-invalid @enderror">
                  @error('cara_bayar_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
