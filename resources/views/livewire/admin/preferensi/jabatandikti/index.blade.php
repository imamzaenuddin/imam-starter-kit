...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jabatandikti', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'keterangan', 'def', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jabatandikti', 'dapat_ubah')) abort(403);

        $item = Jabatandikti::findOrFail($id);
        $this->editId = $item->jabatan_dikti_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Jabatandikti::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jabatandikti::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jabatandikti', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Jabatandikti::query()
            ->when($this->search, function ($query) {
                // $query->where('jabatan_dikti_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jabatan_dikti_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Jabatandikti')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jabatandikti</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jabatandikti</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jabatan_dikti_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jabatan_dikti_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jabatandikti</h5>
