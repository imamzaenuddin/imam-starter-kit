...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/rfidtime', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'jam_mulai', 'jam_selesai', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/rfidtime', 'dapat_ubah')) abort(403);

        $item = Rfidtime::findOrFail($id);
        $this->editId = $item->rfid_time_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Rfidtime::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Rfidtime::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/rfidtime', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Rfidtime::query()
            ->when($this->search, function ($query) {
                // $query->where('rfid_time_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('rfid_time_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Rfidtime')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Rfidtime</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Rfidtime</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->rfid_time_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->rfid_time_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Rfidtime</h5>
