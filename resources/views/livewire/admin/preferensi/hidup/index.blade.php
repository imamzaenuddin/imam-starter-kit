...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/hidup', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'hidup_id', 'nama', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/hidup', 'dapat_ubah')) abort(403);

        $item = Hidup::findOrFail($id);
        $this->editId = $item->hidup_id;
        $this->hidup_id = $item->hidup_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        $rules = [
            'hidup_id' => 'required|string|max:255',
            'nama' => 'nullable',
            'is_aktif' => 'boolean'
        ];
        ...
        $payload = [
            'hidup_id' => $data['hidup_id'],
            'nama' => $data['nama'],
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
        ];

        if ($this->editId) {
            $item = Hidup::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Hidup::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/hidup', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Hidup::query()
            ->when($this->search, function ($query) {
                // $query->where('hidup_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('hidup_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Hidup')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Hidup</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Hidup</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->hidup_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->hidup_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Hidup</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Hidup Id</label>
                  <input wire:model="hidup_id" type="text" class="form-control @error('hidup_id') is-invalid @enderror">
                  @error('hidup_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
