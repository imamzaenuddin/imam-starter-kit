<?php

use App\Models\Jenisdosen;
use App\Services\LogAktivitasService;
use Livewire\Attributes\Layout;
use Livewire\Volt\Component;
use Livewire\WithPagination;

new #[Layout('components.layouts.app')] class extends Component {
    use WithPagination;

    public string $search = '';
    public bool $showModal = false;
    public ?string $editId = null;

    public string $id_legacy = '';
    public string $nama = '';
    public bool $def = false;
    public bool $is_aktif = true;
    public string $no_urut_biaya = '';

    protected $queryString = ['search'];

    public function updatingSearch(): void
    {
        $this->resetPage();
    }

    public function buka(): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisdosen', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'id_legacy', 'nama', 'def', 'is_aktif', 'no_urut_biaya']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisdosen', 'dapat_ubah')) abort(403);

        $item = Jenisdosen::findOrFail($id);
        $this->editId = $item->jenis_dosen_id;
        $this->id_legacy = $item->id_legacy;
        $this->nama = $item->nama;
        $this->def = (bool)$item->def;
        $this->is_aktif = ($item->na === 'N');
        $this->no_urut_biaya = $item->no_urut_biaya;
        $this->showModal = true;
    }

    public function simpan(): void
    {
        if ($this->editId) {
            if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisdosen', 'dapat_ubah')) abort(403);
        } else {
            if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisdosen', 'dapat_buat')) abort(403);
        }

        $rules = [
            'id_legacy' => 'nullable',
            'nama' => 'nullable',
            'def' => 'boolean',
            'is_aktif' => 'boolean',
            'no_urut_biaya' => 'nullable'
        ];

        $data = $this->validate($rules);

        $payload = [
            'id_legacy' => $data['id_legacy'],
            'nama' => $data['nama'],
            'def' => empty($data['def']) ? 0 : 1,
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
            'no_urut_biaya' => $data['no_urut_biaya']
        ];

        if ($this->editId) {
            $item = Jenisdosen::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jenisdosen::create($payload);
            session()->flash('sukses', 'Data baru berhasil ditambahkan.');
        }

        $this->showModal = false;
        $this->reset(['editId', 'id_legacy', 'nama', 'def', 'is_aktif', 'no_urut_biaya']);
        $this->resetPage();
    }

    public function hapus($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisdosen', 'dapat_hapus')) abort(403);

        $item = Jenisdosen::findOrFail($id);
        $item->delete();

        session()->flash('sukses', 'Data berhasil dihapus.');
        $this->resetPage();
    }

    public function with(): array
    {
        $data = Jenisdosen::query()
            ->when($this->search, function ($query) {
                // $query->where('jenis_dosen_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenis_dosen_id', 'desc')
            ->paginate(10);

        return [
            'rows' => $data
        ];
    }
};
?>

@section('title', 'Kelola Jenisdosen')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenisdosen</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenisdosen</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
    <button class="btn btn-primary" wire:click="buka">
      <i class="bx bx-plus me-1"></i> Tambah Baru
    </button>
  </div>

  @if (session('sukses'))
    <div class="alert alert-success alert-dismissible fade show" role="alert">
      {!! session('sukses') !!}
      <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
  @endif

  <div class="card mb-4 border-0 shadow-sm">
    <div class="card-body py-3">
      <div class="input-group input-group-merge">
        <span class="input-group-text"><i class="bx bx-search text-muted"></i></span>
        <input wire:model.live.debounce.300ms="search" type="search" class="form-control" placeholder="Cari data...">
      </div>
    </div>
  </div>

  <div class="card border-0 shadow-sm">
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0">
        <thead class="table-light">
          <tr>
            <th>Id Legacy</th>
            <th>Nama</th>
            <th>Def</th>
            <th>Na</th>
            <th>No Urut Biaya</th>

            <th class="text-center">Aksi</th>
          </tr>
        </thead>
        <tbody>
          @forelse ($rows as $row)
            <tr>
              <td>{{ $row->id_legacy }}</td>
              <td>{{ $row->nama }}</td>
              <td>{{ $row->def }}</td>
              <td>
                @if ($row->na === 'N')
                  <span class="badge bg-label-success">Aktif</span>
                @else
                  <span class="badge bg-label-danger">Tidak Aktif</span>
                @endif
              </td>
              <td>{{ $row->no_urut_biaya }}</td>

              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenis_dosen_id }}')" title="Edit">
                    <i class="bx bx-edit-alt"></i>
                  </button>
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          @click="Swal.fire({
                            title: 'Hapus Data?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Ya, Hapus'
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenis_dosen_id }}'))">
                    <i class="bx bx-trash"></i>
                  </button>
                </div>
              </td>
            </tr>
          @empty
            <tr>
              <td colspan="10" class="text-center py-5 text-muted">Tidak ada data ditemukan.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
    </div>
    @if ($rows->hasPages())
      <div class="card-footer border-top">{{ $rows->links() }}</div>
    @endif
  </div>

  @if ($showModal)
    <div class="modal fade show d-block" tabindex="-1" style="background:rgba(0,0,0,.5)">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenisdosen</h5>
            <button type="button" class="btn-close" wire:click="$set('showModal', false)"></button>
          </div>
          <form wire:submit="simpan">
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Id Legacy</label>
                  <input wire:model="id_legacy" type="text" class="form-control @error('id_legacy') is-invalid @enderror">
                  @error('id_legacy') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Nama</label>
                  <input wire:model="nama" type="text" class="form-control @error('nama') is-invalid @enderror">
                  @error('nama') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Def</label>
                  <div class="form-check form-switch mt-2">
                    <input wire:model="def" class="form-check-input" type="checkbox" id="def_{{ str()->random(4) }}">
                    <label class="form-check-label" for="def_{{ str()->random(4) }}">Def / Default</label>
                  </div>
                  @error('def') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Na</label>
                  <div class="form-check">
                    <input wire:model="na" type="radio" value="1" class="form-check-input">
                    <label class="form-check-label">Option 1</label>
                  </div>
                  @error('is_aktif') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">No Urut Biaya</label>
                  <div class="form-check">
                    <input wire:model="no_urut_biaya" type="radio" value="1" class="form-check-input">
                    <label class="form-check-label">Option 1</label>
                  </div>
                  @error('no_urut_biaya') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

              </div>
            </div>
            <div class="modal-footer border-0 pt-0">
              <button type="button" class="btn btn-outline-secondary" wire:click="$set('showModal', false)">Batal</button>
              <button type="submit" class="btn btn-primary" wire:loading.attr="disabled">
                <span wire:loading.remove wire:target="simpan"><i class="bx bx-save me-1"></i>Simpan</span>
                <span wire:loading wire:target="simpan"><span class="spinner-border spinner-border-sm me-1"></span>...</span>
              </button>
            </div>
          </form>
        </div>
      </div>
    </div>
  @endif
</div>