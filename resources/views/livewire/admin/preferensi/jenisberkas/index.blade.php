...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisberkas', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'bentuk', 'type', 'ukuran', 'is_aktif', 'wajib']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisberkas', 'dapat_ubah')) abort(403);

        $item = Jenisberkas::findOrFail($id);
        $this->editId = $item->jenis_berkas_id;
        $this->nama = $item->nama;
        $this->bentuk = $item->bentuk;
        $this->type = $item->type;
        $this->ukuran = $item->ukuran;
        $this->is_aktif = ($item->na === 'N');
        $this->wajib = $item->wajib;
        $this->showModal = true;
    }
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'bentuk' => ($data['bentuk'] === '' ? null : $data['bentuk']),
            'type' => ($data['type'] === '' ? null : $data['type']),
            'ukuran' => ($data['ukuran'] === '' ? null : $data['ukuran']),
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
            'wajib' => ($data['wajib'] === '' ? null : $data['wajib'])
        ];

        if ($this->editId) {
            $item = Jenisberkas::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Jenisberkas::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenisberkas', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Jenisberkas::query()
            ->when($this->search, function ($query) {
                // $query->where('jenis_berkas_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenis_berkas_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Jenisberkas')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenisberkas</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenisberkas</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenis_berkas_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenis_berkas_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenisberkas</h5>
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="bentuk" type="text" class="form-control @error('bentuk') is-invalid @enderror">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="ukuran" type="text" class="form-control @error('ukuran') is-invalid @enderror">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <div class="form-check">
                    <input wire:model="wajib" type="radio" value="1" class="form-check-input">
