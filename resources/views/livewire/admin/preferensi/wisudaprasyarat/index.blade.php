...
use App\Models\Wisudaprasyarat;
...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/wisudaprasyarat', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'kode_id', 'nama', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/wisudaprasyarat', 'dapat_ubah')) abort(403);

        $item = Wisudaprasyarat::findOrFail($id);
        $this->editId = $item->prasyarat_id;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Wisudaprasyarat::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Wisudaprasyarat::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/wisudaprasyarat', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Wisudaprasyarat::query()
            ->when($this->search, function ($query) {
                // $query->where('prasyarat_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('prasyarat_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Wisudaprasyarat')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Wisudaprasyarat</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Wisudaprasyarat</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->prasyarat_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->prasyarat_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Wisudaprasyarat</h5>
