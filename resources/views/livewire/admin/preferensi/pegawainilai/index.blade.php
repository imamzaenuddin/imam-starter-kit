...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pegawainilai', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'kode_id', 'nama', 'bobot', 'deskripsi', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pegawainilai', 'dapat_ubah')) abort(403);

        $item = Pegawainilai::findOrFail($id);
        $this->editId = $item->pegawai_nilai_id;
        $this->kode_id = $item->kode_id;
        $this->nama = $item->nama;
        $this->bobot = $item->bobot;
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'bobot' => ($data['bobot'] === '' ? null : $data['bobot']),
            'deskripsi' => ($data['deskripsi'] === '' ? null : $data['deskripsi']),
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
        ];

        if ($this->editId) {
            $item = Pegawainilai::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Pegawainilai::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/pegawainilai', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Pegawainilai::query()
            ->when($this->search, function ($query) {
                // $query->where('pegawai_nilai_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('pegawai_nilai_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Pegawainilai')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Pegawainilai</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Pegawainilai</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->pegawai_nilai_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->pegawai_nilai_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Pegawainilai</h5>
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="bobot" type="text" class="form-control @error('bobot') is-invalid @enderror">
                  @error('bobot') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Deskripsi</label>
                  <input wire:model="deskripsi" type="text" class="form-control @error('deskripsi') is-invalid @enderror">
