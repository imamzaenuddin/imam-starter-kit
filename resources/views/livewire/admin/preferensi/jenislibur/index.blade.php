...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenislibur', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'warna', 'keterangan', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenislibur', 'dapat_ubah')) abort(403);

        $item = Jenislibur::findOrFail($id);
        $this->editId = $item->jenis_libur_id;
        $this->nama = $item->nama;
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/jenislibur', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Jenislibur::query()
            ->when($this->search, function ($query) {
                // $query->where('jenis_libur_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('jenis_libur_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Jenislibur')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Jenislibur</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Jenislibur</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->jenis_libur_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->jenis_libur_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Jenislibur</h5>
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="warna" type="text" class="form-control @error('warna') is-invalid @enderror">
