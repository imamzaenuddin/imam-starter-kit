...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/penghasilanortu', 'dapat_buat')) abort(403);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/penghasilanortu', 'dapat_ubah')) abort(403);

        $item = Penghasilanortu::findOrFail($id);
        $this->editId = $item->penghasilan_ortu_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Penghasilanortu::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Penghasilanortu::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/penghasilanortu', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Penghasilanortu::query()
            ->when($this->search, function ($query) {
                // $query->where('penghasilan_ortu_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('penghasilan_ortu_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Penghasilanortu')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Penghasilanortu</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Penghasilanortu</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->penghasilan_ortu_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->penghasilan_ortu_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Penghasilanortu</h5>
