...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/unitorganisasi', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'kode_id', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/unitorganisasi', 'dapat_ubah')) abort(403);

        $item = Unitorganisasi::findOrFail($id);
        $this->editId = $item->unit_organisasi_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Unitorganisasi::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Unitorganisasi::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/unitorganisasi', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Unitorganisasi::query()
            ->when($this->search, function ($query) {
                // $query->where('unit_organisasi_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('unit_organisasi_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Unitorganisasi')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Unitorganisasi</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Unitorganisasi</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->unit_organisasi_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->unit_organisasi_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Unitorganisasi</h5>
