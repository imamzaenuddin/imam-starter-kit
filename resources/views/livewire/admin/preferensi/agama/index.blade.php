...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/agama', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'id_legacy', 'agama_id', 'nama', 'is_aktif', 'kode_id']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/agama', 'dapat_ubah')) abort(403);

        $item = Agama::findOrFail($id);
        $this->editId = $item->agama_id;
        $this->id_legacy = $item->id_legacy;
        $this->agama_id = $item->agama_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        if ($this->editId) {
            $item = Agama::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Agama::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/agama', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Agama::query()
            ->when($this->search, function ($query) {
                // $query->where('agama_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('agama_id', 'desc')
            ->paginate(10);
...
<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Agama</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data referensi <strong>Agama</strong>. Pastikan data akurat karena akan digunakan di seluruh modul sistem.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->agama_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->agama_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Agama</h5>
          ...
          <form wire:submit="simpan">
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Agama Id</label>
                  <input wire:model="agama_id" type="text" class="form-control @error('agama_id') is-invalid @enderror">
