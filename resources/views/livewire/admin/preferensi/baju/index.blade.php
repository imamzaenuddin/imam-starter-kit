...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/baju', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'baju_id', 'nama', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/baju', 'dapat_ubah')) abort(403);

        $item = Baju::findOrFail($id);
        $this->editId = $item->baju_id;
        $this->baju_id = $item->baju_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        $rules = [
            'baju_id' => 'required|string|max:255',
            'nama' => 'nullable',
            'is_aktif' => 'boolean'
        ];
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/baju', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Baju::query()
            ->when($this->search, function ($query) {
                // $query->where('baju_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('baju_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Baju')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Baju</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Baju</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->baju_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->baju_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Baju</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Baju Id</label>
                  <input wire:model="baju_id" type="text" class="form-control @error('baju_id') is-invalid @enderror">
                  @error('baju_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
