...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/saat', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'saat_id', 'nama', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/saat', 'dapat_ubah')) abort(403);

        $item = Saat::findOrFail($id);
        $this->editId = $item->saat_id;
        $this->saat_id = $item->saat_id;
        $this->nama = $item->nama;
    ...
    public function simpan(): void
    {
        ...
        $rules = [
            'saat_id' => 'required|string|max:255',
            'nama' => 'nullable',
            'is_aktif' => 'boolean'
        ];
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/saat', 'dapat_hapus')) abort(403);
    ...
    public function with(): array
    {
        $data = Saat::query()
            ->when($this->search, function ($query) {
                // $query->where('saat_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('saat_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Saat')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Saat</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Saat</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->saat_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->saat_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Saat</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Saat Id</label>
                  <input wire:model="saat_id" type="text" class="form-control @error('saat_id') is-invalid @enderror">
                  @error('saat_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
