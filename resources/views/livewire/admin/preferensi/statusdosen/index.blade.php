...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    public ?string $status_dosen_id = null;
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusdosen', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'status_dosen_id', 'no_id', 'nama', 'def', 'honor_mengajar', 'is_aktif', 'grade_nilai', 'nilai_ujian_max']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusdosen', 'dapat_ubah')) abort(403);

        $item = Statusdosen::findOrFail($id);
        $this->editId = $item->status_dosen_id;
        $this->status_dosen_id = $item->status_dosen_id;
    ...
    public function simpan(): void
    {
        ...
        $rules = [
            'status_dosen_id' => 'required|string|max:255',
            'no_id' => 'nullable',
            'nama' => 'nullable',
            'def' => 'boolean',
        ...
        $payload = [
            'status_dosen_id' => ($data['status_dosen_id'] === '' ? null : $data['status_dosen_id']),
        ...
        if ($this->editId) {
            $item = Statusdosen::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Statusdosen::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusdosen', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Statusdosen::query()
            ->when($this->search, function ($query) {
                // $query->where('status_dosen_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('status_dosen_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Statusdosen')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Statusdosen</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Statusdosen</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->status_dosen_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->status_dosen_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Statusdosen</h5>
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Status Dosen Id</label>
                  <input wire:model="status_dosen_id" type="text" class="form-control @error('status_dosen_id') is-invalid @enderror">
                  @error('status_dosen_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <div class="form-check">
                    <input wire:model="nilai_ujian_max" type="radio" value="1" class="form-check-input">
