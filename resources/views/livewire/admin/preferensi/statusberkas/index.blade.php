...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusberkas', 'dapat_buat')) abort(403);

        $this->reset(['editId', 'nama', 'verifikasi', 'status', 'is_aktif']);
        $this->showModal = true;
    }

    public function edit($id = null): void
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusberkas', 'dapat_ubah')) abort(403);

        $item = Statusberkas::findOrFail($id);
        $this->editId = $item->status_berkas_id;
        $this->nama = $item->nama;
        $this->verifikasi = $item->verifikasi;
    ...
    public function simpan(): void
    {
        ...
        $payload = [
            ...
            'verifikasi' => ($data['verifikasi'] === '' ? null : $data['verifikasi']),
            'status' => ($data['status'] === '' ? null : $data['status']),
            'na' => empty($data['is_aktif']) ? 'Y' : 'N',
        ];

        if ($this->editId) {
            $item = Statusberkas::findOrFail($this->editId);
            $item->update($payload);
            session()->flash('sukses', 'Data berhasil diperbarui.');
        } else {
            $item = Statusberkas::create($payload);
    ...
    {
        if (! auth()->user()?->bisaMenu('/admin/preferensi/statusberkas', 'dapat_hapus')) abort(403);
    ...
    {
        $data = Statusberkas::query()
            ->when($this->search, function ($query) {
                // $query->where('status_berkas_id', 'like', '%' . $this->search . '%');
            })
            ->orderBy('status_berkas_id', 'desc')
            ->paginate(10);
...
@section('title', 'Kelola Statusberkas')

<div>
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h4 class="fw-bold mb-1"><i class="bx bx-table me-2 text-primary"></i>Kelola Statusberkas</h4>
      <p class="text-muted small mb-0">Halaman manajemen sentral untuk data <strong>Statusberkas</strong>. Anda dapat melakukan pencarian, penambahan, hingga modifikasi data di sini.</p>
    </div>
            ...
            <tr>
              ...
              <td class="text-center">
                <div class="d-flex gap-1 justify-content-center">
                  <button class="btn btn-sm btn-icon btn-text-primary" wire:click="edit('{{ $row->status_berkas_id }}')" title="Edit">
                  ...
                  <button class="btn btn-sm btn-icon btn-text-danger" title="Hapus"
                          ...
                          }).then(r => r.isConfirmed && $wire.hapus('{{ $row->status_berkas_id }}'))">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
          <div class="modal-header border-0 pb-0">
            <h5 class="modal-title fw-bold">{{ $editId ? 'Ubah' : 'Tambah' }} Statusberkas</h5>
            ...
            <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12 mb-3">
                  ...
                  <input wire:model="verifikasi" type="text" class="form-control @error('verifikasi') is-invalid @enderror">
                  @error('verifikasi') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12 mb-3">
                  <label class="form-label fw-semibold">Status</label>
                  <input wire:model="status" type="text" class="form-control @error('status') is-invalid @enderror">
